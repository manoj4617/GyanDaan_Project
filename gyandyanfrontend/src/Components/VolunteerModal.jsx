...
export default function VolunteerModal(props) {
  ...
  const acceptRequest = (volunteerId, reqId, studentID) => {
    httpClient
      .get(
        `RequirementTranscation/accept-request/${volunteerId}/${reqId}/${studentID}`
      )
      .then((res) => {
  ...
  const rejectRequest = (volunteerId, reqId, studentID) => {
    httpClient
      .get(
        `RequirementTranscation/reject-request/${volunteerId}/${reqId}/${studentID}`
      )
      .then((res) => {
        setmessage(res.data);
      })
              ...
              {props.invites.map((item, index) => (
                <ul className="list-group">
                  <li className="list-group-item fs-6 bg-dark text-white">
                    <p>
                      You have a request from {item.studentProfile.firstName}{" "}
                      requesting join {item.volunteer.subject}{" "}
                      {typeOfClass[item.volunteer.typeOfClass]} class
                    </p>
                    <button
                      type="button"
                      onClick={() =>
                        acceptRequest(
                          item.volunteer.volunteerProfileId,
                          item.volunteer.id,
